"""V8 demo: Sunburst with physical specs in prompt.

Single-image Sunburst like v7, but the prompt includes watch specs
(case diameter, band width) and wrist circumference so the model can
reason about correct proportions. Also fixes strap color preservation
and requests glass reflection.
"""
import base64
import gc
import json
...
def harmonize_gpt(
    ...
    prompt = (
        ...
        "The watch was digitally placed from a product photo and needs to be harmonized to look natural. "
        f"\n\nWatch specs: {case_diameter_mm}mm case diameter, {band_width_mm}mm band width. "
        f"Wrist circumference: {circumference_mm}mm. "
        "The watch should sit right at the wrist crease (where hand meets forearm), "
        "not further up the forearm. Adjust the position down toward the hand if needed. "
        f"At this wrist size, a {case_diameter_mm}mm case should look proportional — "
        "if it appears too large, scale it down to match realistic proportions. "
        "Apply natural perspective foreshortening based on the camera angle — "
        "the watch face should not appear perfectly flat/front-on if the wrist is angled. "
        ...
        "and add a subtle glass reflection/glare on the watch crystal to match the ambient light. "
        ...
        "Do not change the watch model or any text on the dial or bezel. "
...
def main():
    ...
    cv2.imwrite(str(output_dir / "v8_composite_raw.png"), composite)
    ...
    out_path = output_dir / "demo_v8.png"
