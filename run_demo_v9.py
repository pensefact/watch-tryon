"""V9 demo: 3-image Sunburst harmonization.

Sends product photo + bare wrist + composite to GPT-Image-2.5 Sunburst,
with physical specs (case diameter, band width, wrist circumference) in
the prompt so the model can correct size, position, and perspective.
"""
import base64
import gc
import json
...
def harmonize_gpt(
    ...
    product_img = cv2.imread(str(product_photo_path))
    product_url = _upload_image(product_img, max_dim=1024)
    bare_wrist_url = _upload_image(bare_wrist, max_dim=2048)
    composite_url = _upload_image(composite, max_dim=2048)

    prompt = (
        "You are given three images:\n"
        "1. A product photo of a watch (clean reference showing exact colors and details)\n"
        "2. A bare wrist photo (the target scene with its lighting and skin tones)\n"
        "3. A composite where the watch has been digitally placed onto the wrist\n\n"
        f"The watch is a {watch_name}: {case_diameter_mm}mm case diameter, "
        f"{band_width_mm}mm band width. The wrist circumference is {circumference_mm}mm.\n\n"
        "Edit the third image (the composite) to make the watch look naturally worn:\n"
        "- Match the lighting and color temperature from the bare wrist scene\n"
        "- Soften strap-to-skin edges and add natural shadows under case and strap\n"
        "- Make the strap wrap naturally around the wrist\n"
        "- Add subtle glass reflection/glare on the crystal matching the ambient light\n"
        "- The watch should sit at the wrist crease — adjust position toward the hand if needed\n"
        f"- A {case_diameter_mm}mm watch on a {circumference_mm}mm wrist should look proportional — "
        "adjust size if it appears too large\n"
        "- Apply natural perspective foreshortening for the camera angle\n\n"
    ...
    print(f"    Sending 3 images to GPT-Image-2.5 Sunburst via fal.ai (quality={quality})...")
    print(f"    Prompt: {prompt[:120]}...")

    result = fal_client.subscribe("openai/gpt-image-2.5/sunburst/edit", arguments={
        "prompt": prompt,
        "image_urls": [product_url, bare_wrist_url, composite_url],
        "quality": quality,
        "num_images": 1,
        "output_format": "png",
    })
...
def main():
    ...
    cv2.imwrite(str(output_dir / "v9_composite_raw.png"), composite)
    print(f"  Raw composite saved")

    # --- Step 4: GPT-Image harmonization ---
    print("\n=== Harmonization (GPT-Image-2.5 Sunburst) ===")
    try:
        harmonized = harmonize_gpt(
            composite, WATCH_REF, wrist_image, spec.name,
            case_diameter_mm=spec.case_diameter_mm,
            band_width_mm=spec.band_width_mm,
            circumference_mm=circumference_mm,
        )
        cv2.imwrite(str(output_dir / "v9_harmonized.png"), harmonized)
        final = harmonized
    ...
    out_path = output_dir / "demo_v9.png"
