"""V10 demo: 2-image Sunburst harmonization (composite + bare wrist).

Sends bare wrist + composite to GPT-Image-2.5 Sunburst (no product photo),
with physical specs in the prompt. Tests whether the bare wrist context
alone is enough for good lighting/integration without the product reference.
"""
import base64
import gc
import json
...
def harmonize_gpt(
    ...
    prompt = (
        ...
        "1. A bare wrist photo (the target scene with its lighting and skin tones)\n"
        "2. A composite where a watch has been digitally placed onto the same wrist\n\n"
        ...
        "Edit the second image (the composite) to make the watch look naturally worn. "
        "Use the bare wrist photo as your lighting/scene reference:\n"
        ...
        "preserve this grey-green strap color exactly, do not darken it to black."
    )

    print(f"    Sending 2 images to GPT-Image-2.5 Sunburst via fal.ai (quality={quality})...")
    print(f"    Prompt: {prompt[:120]}...")

    result = fal_client.subscribe("openai/gpt-image-2.5/sunburst/edit", arguments={
        "prompt": prompt,
        "image_urls": [bare_wrist_url, composite_url],
        "quality": quality,
        "num_images": 1,
        "output_format": "png",
    })
...
def main():
    ...
    cv2.imwrite(str(output_dir / "v10_composite_raw.png"), composite)
    ...
    out_path = output_dir / "demo_v10.png"
